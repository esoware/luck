...
description: Changes how luck_formatter formats a Lua/Luau construct, covering layout, packing, breaking, hug patterns, and comment placement, while preserving the idempotency invariant. Use when asked to format X differently, wrap long calls, fix indentation, or for any edit under crates/luck_formatter/.
---
...
- **Comment safety.** If the construct can contain comments (tables, arg
  ...
  relocation is this crate's historical worst bug class. `emit_trailing_comments`
  drains the comments no emitter visited, after the statement.
...
- **Types are real AST** (`luck_ast::types::Type`), so extend the
  `format_type.rs` impls and never re-tokenize text.
