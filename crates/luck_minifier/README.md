...
- **Fixpoint pipeline** — `fold_constants` and `remove_dead_code` each run twice within a round (inlining exposes new folds, folding exposes new dead branches), and the whole chain iterates until the emitted output stops changing, not a single fixed pass.
...
5. `fold_constants` — second pass; inlining exposes new folds.
6. `remove_dead_code` — second pass; the newly exposed folds (e.g. `local DEBUG = false` inlined into `if DEBUG then`) expose new dead branches.
