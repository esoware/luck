...
- **Programmatic synthesis** — the `synth` module (`luck_ast::synth::Synth`) builds AST nodes without any source text, handing out fresh monotonic dummy spans. It targets tools that emit an AST directly (e.g. a decompiler backend) and carries `SyntheticComment` for node-anchored comment attachment. `builder.rs` provides only `Punctuated<T>` helpers and `span()` accessors — there are no fluent `with_*` constructors.
