...
- **Position-independent requires.** A `require("string_literal")` resolves from anywhere: any statement, any expression position, any function body. The lazy loader makes require order irrelevant, so bare side-effect requires and requires below other code all bundle. A non-string-literal argument (`require(name)`) cannot name a module at build time, so the bundler keeps the call and resolves it at runtime rather than failing the build. See **Dynamic requires** below. The extractor decodes require strings like real Lua string literals, so `require("a\46b")` and `require("a.b")` are the same module.
- **Dynamic requires degrade instead of aborting.** The `dynamic_require` key in `luck.json`, or `--dynamic-require`, governs `require(expr)`. The default `warn` keeps the call and reports W007, `allow` keeps it silently, and `error` restores the hard failure with E002. On Lua targets the emitter retargets a kept call at the loader's `__luck_dynamic`, which serves any name registered in the bundle, since the cache is keyed by module name and a runtime-computed name can hit it. Anything else falls through to the host's own `require`, read at call time. Luau keys its cache by resolved file, and a Roblox `require` takes an Instance, so on those targets the call is left exactly as written.
...
- **No host paths in the output.** Every path the bundle carries, in module provenance comments and the file path 5.2+ chunks receive as loader data, is project-relative. A module above the project root keeps its shape through `..` segments, so the root's own side of the tree, the part holding the build host's home and project directories, is never named. When a module shares no root with the project at all, such as another Windows drive or a UNC share, only its file name is emitted.
...
1. **Require extraction.** `require_extraction.rs` walks each module's entire AST for `require()` calls, using `luck_semantic` scope analysis to skip calls that resolve to local bindings, and decodes each string literal to its runtime value. It produces a list of require sites (decoded string plus spans), the callee spans of the dynamic requires the emitter retargets, and validation diagnostics (W007/E002 non-string-literal argument, E006 `package.loaded` write on Luau, W001 duplicate require, W005 shadowed/aliased require).
